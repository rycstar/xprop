...
#define CHECK(predicate) \
  do { \
    if (!(predicate)) { \
      printf("%s:%d: %s CHECK '" #predicate "' failed", \
...
static constexpr size_t PA_SIZE = 64 * 1024;

#define SERIAL_DIRTY(serial) ((serial)&1)
#define SERIAL_VALUE_LEN(serial) ((serial) >> 24)

static const char property_service_socket[] =PROP_SERVICE_PATH PROP_SERVICE_NAME ;
...
static int send_prop_msg(const prop_msg* msg) {
  ...
  if (num_bytes == sizeof(prop_msg)) {
    ...
    if (poll_result == 1 && (pollfds[0].revents & POLLHUP) != 0) {
      ...
      PROP_LOG(LOG_WARNING, "Property service has timed out while trying to set \"%s\" to \"%s\"",
                            msg->name, msg->value);
      result = 0;
    }
  }
...
bool context_node::open(bool access_rw, bool* fsetxattr_failed) {
  ...
  //int len = async_safe_format_buffer(filename, sizeof(filename), "%s/%s", property_filename,
   //                                  context_);
  int len = snprintf(filename, sizeof(filename), "%s/%s", property_filename,
                                     context_);
...
static bool map_system_property_area(bool access_rw, bool* fsetxattr_failed) {
  ...
  int len =
      snprintf(filename, sizeof(filename), "%s/properties_serial",
                               property_filename);
  if (len < 0 || len > PROP_FILENAME_MAX) {
    __system_property_area__ = nullptr;
    return false;
...
static bool initialize_properties_from_file(const char* filename) {
  ...
  while (getline(&buffer, &line_len, file) > 0) {
    ...
    if (!strncmp(prop_prefix, "import", 7) ) {
        //it will be return false, we ignore it .
            initialize_properties_from_file(context);
            continue;
    }
    /*
...
static bool initialize_properties() {
  ...
  if (initialize_properties_from_file(DEFAULT_PROPERTY_CONTEXTS)) {
    return true;
  }
...
int __attribute__((constructor)) __system_properties_init() {
...
const prop_info* __system_property_find(const char* name) {
  ...
  if (!pa) {
        PROP_LOG(LOG_ERR,"Access denied finding property \"%s\"", name);
    return nullptr;
  }
...
#endif
int __system_property_read(const prop_info* pi, char* name, size_t nlen, char* value, size_t vlen) {
  while (true) {
    uint32_t serial = __system_property_serial(pi);  // acquire semantics
    size_t len = SERIAL_VALUE_LEN(serial);
    if (len > vlen){
        PROP_LOG(LOG_ERR,"actual len is %d, but value len %d, cp failed\n", len, vlen);
        return 0;
    }
    ...
    if (serial == load_const_atomic(&(pi->serial), memory_order_relaxed)) {
      if (name != nullptr) {
        size_t namelen = strlcpy(name, pi->name, MIN(nlen,(strlen(pi->name)+1)));
        if (namelen >= nlen) {
          PROP_LOG(LOG_ERR,     "The property name length for \"%s\" is >= %d;"
...
int __system_property_get(const char* name, char* value, size_t vlen) {
  const prop_info* pi = __system_property_find(name);

  if (pi != 0) {
    return __system_property_read(pi, nullptr,0, value, vlen);
  } else {
    value[0] = 0;
    return 0;
  }
...
static void detect_protocol_version() {
  char value[PROP_VALUE_MAX];
  if (__system_property_get(kServiceVersionPropertyName, value,sizeof(value)) == 0) {
    g_propservice_protocol_version = kProtocolVersion2;
    PROP_LOG(LOG_WARNING,"Using old property service protocol (\"%s\" is not set)",
    ...
    if (version >= kProtocolVersion2) {
     ...
     PROP_LOG(LOG_WARNING, "Using old property service protocol (\"%s\"=\"%s\")",
...
int __system_property_add(const char* name, unsigned int namelen, const char* value,
  ...
  if (!pa) {
    PROP_LOG(LOG_ERR,  "Access denied adding property \"%s\"", name);
    return -1;
  }
...
uint32_t __system_property_serial(const prop_info* pi) {
  ...
  while (SERIAL_DIRTY(serial)) {
    __futex_wait(const_cast< atomic_uint_least32_t volatile *>(&pi->serial), serial, nullptr);
...
#if 0
int system_prop_get (const char* key, const char* value, size_t vlen){
   // __system_property_get(key, (char *)value, vlen);
}
void *system_prop_get (const char* key, const char* value, size_t vlen){
   // __system_property_get(key, (char *)value, vlen);
   return NULL;
}
#endif
...
extern "C" { 
#endif
bool system_prop_wait(const prop_info* pi,
                            uint32_t old_serial,
                            uint32_t* new_serial_ptr,
                            const unsigned int ms){
    struct timespec ts; if (ms > 0)
    { 
        ts.tv_sec = ms / 1000; 
        ts.tv_nsec = (ms%1000)*1000 * 1000;    
        return __system_property_wait(pi, old_serial, new_serial_ptr, &ts);
    }else{
        return __system_property_wait(pi, old_serial, new_serial_ptr, NULL);
    }

}

extern int system_prop_set (const char* key, const char* value) __attribute ((alias ("__system_property_set")));
extern int system_prop_get (const char* key, const char* value, size_t vlen) __attribute ((alias ("__system_property_get")));
extern int system_prop_foreach(void (*propfn)(const void* pi, void* cookie), void* cookie) __attribute ((alias ("__system_property_foreach")));

extern const prop_info* system_prop_find(const char* name) __attribute ((alias ("__system_property_find")));
extern void system_prop_read_callback(const void *pi,
    void (*callback)(void* cookie, const char *name, const char *value, uint32_t serial),
    void* cookie) __attribute ((alias ("__system_property_read_callback")));
#if 0
extern bool system_prop_wait(const prop_info* pi,
                            uint32_t old_serial,
                            uint32_t* new_serial_ptr,
                            const struct timespec* relative_timeout) __attribute ((alias ("__system_property_wait")));
#endif

/* Deprecated. Use __system_property_read_callback instead. */
extern int system_prop_read(const prop_info* pi, char* name,size_t nlen, char* value,size_t vlen) __attribute ((alias ("__system_property_read")));
/* Deprecated. Use __system_property_read_callback instead. */
extern const prop_info* system_prop_find_nth(unsigned n) __attribute ((alias ("__system_property_find_nth")));
extern int system_prop_get_pi_info(const void * pi_, char * name, size_t name_len, char * val, size_t val_len) __attribute ((alias ("__system_property_read")));;
extern uint32_t system_prop_get_pi_serial(const void * pi_) __attribute ((alias ("__system_property_serial"))); 
#ifdef __cplusplus  
}  
#endif 
